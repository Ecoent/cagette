...
var isomorphicUnfetch = commonjsGlobal.fetch = commonjsGlobal.fetch || (
	typeof process=='undefined' ? (require$$0.default || require$$0) : (function(url, opts) {
		return require$$1(url.replace(/^\/\//g,'https://'), opts);
	})
);
...
var CustomTextField = function (props) {
    ...
    var setFieldValue = props.form.setFieldValue, name = props.field.name;
    var onChange = React__default.useCallback(function (event) {
        var value = event.target.value;
        setFieldValue(name, value ? value.toUpperCase() : '');
    }, [setFieldValue, name]);
    var muiProps = formikMaterialUi.fieldToTextField(props);
    if (muiProps.helperText) {
        var key = void 0;
        var values = {};
        if (typeof muiProps.helperText === 'object') {
            key = muiProps.helperText.key;
            values = muiProps.helperText.values;
        }
        else {
            key = muiProps.helperText;
        }
        muiProps = __assign(__assign({}, muiProps), { helperText: t(key, values) });
    }
    return React__default.createElement(core.TextField, __assign({}, muiProps, { onChange: onChange }));
};
